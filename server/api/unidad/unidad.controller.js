...
exports.update = function(req, res) {
	...
	for (var i = 0; i < input.unidad.variables.length; i++) {
		if(typeof input.unidad.variables[i].value  !== "undefined"){
			multiplequery+='REPLACE INTO unidad_variable SET valor='+connection.escape(input.unidad.variables[i].value)+',unidad_id = '+connection.escape(data.un_id)+',variable_id='+connection.escape(input.unidad.variables[i].idvariable)+';';
		}
	};
	for (var i = 0; i < input.unidad.variablesindependientes.length; i++) {
		if(typeof input.unidad.variablesindependientes[i].valueSelected  !== "undefined"){
			multiplequery+='REPLACE INTO unidad_variableindependiente SET value='+connection.escape(input.unidad.variablesindependientes[i].valueSelected)+',unidad_id = '+connection.escape(data.un_id)+', variable_id='+connection.escape(input.unidad.variablesindependientes[i].varind_id)+';';
		}
	};
